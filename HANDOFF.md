...
## 15. 2026-09-13セッションでの追加実装（ver.6.0 → 追加要望9件＋続けて3件）

前回セッション（ver.6.0として納品済み）に対して、2回に分けて追加要望をいただいた。
1回目の9項目と2回目の3項目をあわせてこのセッションで実装した。

### ①並び替え問題のグループ化記号を「#」→「*」に変更
- `js/questionSystem.js`の`tokenizeForReorder`を、`*`で囲まれた範囲を1トークンとして
  まとめる仕様に変更（従来の`#`実装から delimiter を差し替えただけで、ロジックは同じ）。
  例：`*古代→**中世→**近世→**近代→*` → `["古代→","中世→","近世→","近代→"]`（4択）。
- Lv1（正誤）・Lv2（四択）・タイピング復習など、並び替え以外の場面では`*`を除去して
  表示・照合するよう`QuestionSystem.stripReorderMarkers()`を新設し、`questionSystem.js`
  （Lv1/Lv2生成）・`reviewSystem.js`（1日後/3日後タイピング、☆フォルダタイピング）・
  `utils.js`の`formatAnswerValue`（保険的に最終表示直前でも除去）に適用した。

### ②動画の縦横切替
- 確認したところ、前々回セッション（ver.6.0以前）で既に実装済みで、ご要望の内容
  （縦画面で`_mobile`付き、横画面で通常版、`open.mp4`/`gacha.mp4`とも対応）と完全に
  一致していたため、コード変更は行っていない（`js/app.js`のplayCutscene参照）。

### ③CSV列順変更
- `js/csvManager.js`のHEADER配列・toCSV・toJSONの出力順を
  `id,genre,question_set,stage,question,answer,antonym,synonyms,unrelated,note,generate_question`
  に変更（noteとgenerate_questionを入れ替え）。インポートは列名ベースで読むため影響なし。

### ④ステージ図形の霧を画像化
- `js/fogSystem.js`に`stageFogImage(progress)`を追加。denceFog.png（未開始）→
  lightFog.png（Lv1クリア）→fogYellow.png（Lv2クリア）→画像なし（Lv3クリア）の順。
- `js/mapSystem.js`でSVGの`<clipPath>`を使い、ステージ図形（○△□☆）の輪郭の形に
  画像を切り抜いて重ねる方式に変更。不透明度は仕様通り一律30%固定
  （`FogSystem.STAGE_FOG_IMAGE_OPACITY`）。画像読み込み失敗時は静かに要素を削除する
  だけで、地図描画自体は止めない。

### ⑤並び替えカードのタップ挙動
- `js/quizSystem.js`の`renderReorderUI`を書き直し：
  - 解答欄に並んだカードもボタンとして表示し、再タップで解答欄から取り消せる
    （元のプール内スロットへ自動的に戻る）。
  - プール側は使用済みスロットを詰めずに空けたままにする（`.char-chip-empty`という
    見えないプレースホルダーで場所を確保する）。

### ⑥ログインの1日1回化
- `js/gameState.js`に`hasLoggedInToday()`を追加（stateを書き換えずストレージだけを
  確認する）。`js/app.js`の`boot()`を非同期化し、今日すでにログイン済みなら
  タイトル画面を省略してそのままホーム画面へ進むようにした（扉の演出も、毎回見ると
  煩わしいためこの自動続行時のみ省略している）。日付が変わればまた通常のログイン
  （バックアップファイル選択／この端末の保存データで続ける）が必要になる。

### ⑦ホーム「はじめる」ボタンの遷移先変更
- `js/mapSystem.js`に`findNextIncompleteStageTarget()`を追加（進捗100%未満の
  「道」をジャンル→道の順に探して最初の1件を返す）。
- ホーム画面の「はじめる」ボタンは、**復習が溜まっている場合は従来通り「知の探究」を
  優先**し、そうでない場合は上記の道のステージ地図（areaMap）へ直接ジャンプするように
  変更した（エリア選択→道選択という手順を1タップ省略する）。
  **【判断が必要な点】** 「復習を常に優先する」という部分は要望に明記されていない
  独自判断。復習の有無に関わらず必ずステージ画面を開く仕様に変更したい場合はご連絡を。

### ⑧横長（ランドスケープ）UI
- `css/responsive.css`に`@media (orientation: landscape)`ブロックを追加。
  - 下部の共通ナビゲーション（ホーム/探索/設定）を右側の縦ツールバーに変更。
  - ホーム画面：「今日のおすすめ行動」と「自分の世界の様子」を横並びに
    （`js/router.js`側で両者を`.home-panels-row`という共通の親要素でまとめ、
    縦画面では従来通り縦積みのまま、横画面のときだけCSSでflex-rowにする）。
  - 探索画面：4つのボタンはレイアウト（配置）はそのまま、サイズだけを拡大。
  - 設定画面：`.settings-screen-inner`クラスを追加し、横画面時のみ画面幅の50%・
    中央寄せにした。
  - 縦画面用のCSSは一切変更していない（`orientation: landscape`の中だけで完結）。

### ⑨追加判断機能：「今日の目標」（コミットメント・デバイス）
- 詳細は`README.md`の「追加判断」セクションに記載。要点：ホーム画面で「1問/3問/5問」を
  自分で宣言し、新規学習・復習いずれのクエスト完了でもカウントが進む。未達成でも
  何も失わない設計（損失回避の悪用防止）。`GameState.dailyGoal`として永続化。
...
### 画面別BGM
- `js/bgmSystem.js`を新規作成。ホーム→home.mp3、探索→explore.mp3、設定→
  settings.mp3、ステージ画面（areaMap）＋問題を解いている間（quiz / reviewPlay /
  starReviewPlay）は一貫してstage.mp3を再生する。`Router.navigate()`から画面名を
  渡して呼び出す方式（`js/router.js`に1行追加）。
- **音声ファイル自体は同梱していない**（`assets/audio/`フォルダとREADME.txtのみ用意）。
  Saito様の方で`assets/audio/home.mp3`等を配置してください。ファイルが無い場合は
  自動再生エラーを握りつぶし、アプリの動作には影響しない。
- 個別に指定の無い画面（日記・ガチャ・称号・道選択・エリア選択など）ではBGMを
  切り替えず、直前まで流れていた曲をそのまま流し続ける仕様にした
  （**独自判断。毎回止める/常に無音にする等、挙動を変えたい場合はご連絡を**）。
- ブラウザの自動再生制限で最初の再生がブロックされた場合、次のクリック操作を
  検知して自動的に再試行するフォールバックを入れている。

### 結果画面のボタン修正・追加
- 「次へ進む」を押すとエリア選択画面まで戻ってしまう問題を修正。`js/resultSystem.js`
  の`renderSuccessScreen`に`GameState.findStageContext(stageId)`で文脈を取得させ、
  - Lv1・Lv2クリア時 → 同じステージのレベル選択ポップアップが開いた状態（areaMapへ
    `openStageId`付きで遷移）
  - Lv3クリア時 → 次のステージのレベル選択ポップアップが開いた状態
    （次のステージが無ければポップアップなしのareaMapへ）
  に直接遷移するようにした。
- あわせて「道の選択」（questionSetSelectへ）・「ステージ選択」（areaMapへ、
  ポップアップなし）ボタンを追加。
- `js/mapSystem.js`の`renderAreaMapScreen`に`params.openStageId`対応を追加
  （指定があれば該当ステージの`openStagePopup`を自動的に開く）。

### 並び替えカードのサイズ調整
- `css/quiz.css`に`@media (orientation: portrait)`ブロックを追加。縦画面では
  `.reorder-pool`をCSS Gridの5列固定（`repeat(5, 1fr)`）にし、各カードは
  `aspect-ratio: 1/1`で正方形を保ったまま拡大、6個目以降は自動的に次の行へ折り返す。
  横画面は元の（このセッション⑤で調整した）システムをそのまま維持している
  （ベースの`.char-chip`定義を変更していないため）。

### 動作確認方法
全JSファイルで`node --check`、全CSSファイルで中括弧の対応数チェックを実施し、
いずれも問題なし。ブラウザでの実機能テスト（音声再生・横画面レイアウト・
ステージ霧画像の見た目など）はサンドボックスの制約により未実施。次回、実機・実
ブラウザでの見た目確認を推奨。

### 次回セッションへの申し送り
- ⑦「はじめる」ボタンの復習優先ロジックが要望通りかどうかの確認。
- BGMの`assets/audio/*.mp3`本体の追加をお願いしたい（無くても動作はする）。
...
## 17. 2026-09-16セッションでの追加実装（10項目の機能追加要望）

このセッションでは、Saito様から提示された10項目の追加要望に対応した。

### ①問題画面の「→」を含む部分の文字色
- `css/quiz.css`の`.quiz-shown-answer`（Lv1で「→ ○○」と表示される部分）の
  `color`を`#4F80AB`（前回セッションでの変更値）から`#000080`へ変更した。

### ②結果画面「カード」ボタンとその下のボタン群の間のmargin
- `js/resultSystem.js`の`renderFailScreen`で、「カード」ボタンに
  `style: "margin-bottom: 20px;"`を追加し、「ステージへ」「再挑戦する」の
  ボタン行との間に余白ができるようにした。

### ③フラッシュカード画面（「カード」ボタンから開く画面）の改修
- `js/resultSystem.js`の`renderFlashcardScreen`を改修：
  - 「正答」面に、実際に出題で使われた選択肢一覧を表示するようにした。
    Lv2（四択）は`item.choices`を、正答をハイライト（緑背景）して一覧表示。
    Lv3（並び替え）は`item.scrambled`（バラバラのトークン）をハイライト表示。
    Lv1（正誤）や上記が無い形式は、従来通り正答の値のみを表示する。
  - カード上でのスワイプ（`touchstart`/`touchend`、横方向40px以上の移動）でも
    前後のカードへ切り替えられるようにした（ボタンでの切り替えは従来通り維持）。
  - 「問題」面のラベルにのみ`.is-question-label`クラスを付与し、
    `css/quiz.css`側で`#000080`を指定した（「正答」面のラベルは変更していない）。
- `css/quiz.css`に`.flashcard-choices-list` / `.flashcard-choice-chip` /
  `.flashcard-choice-chip.is-correct-choice` / `.flashcard-face-label.is-question-label`
  を追加。

### ④ホーム画面「今日のおすすめ行動」と「世界の様子」の配置
- 前回セッション（16章以前）で、横画面（landscape）時のみこの2つを横並びに
  する`responsive.css`の上書きが入っていたが、今回の要望によりこの上書きを撤去した。
  `style.css`側の既定（`.home-panels-row { flex-direction: column; }`）がそのまま
  適用されるため、画面の向きに関わらず常に縦に並ぶ。

### ⑤復習問題として登録されたものもデータ管理での出力に反映
- 原因：1週間後の復習「訂正問題作成」（仕様13章）でユーザーが作成した訂正問題は、
  `state.genres`配下の通常の問題バンクではなく`state.reviewSchedules`の各エントリの
  `correctionQuestion`に保存されていたため、`CsvManager.collectQuestions()`
  （全問題／エリア別／道別／ステージ別の出力が共通で使う関数）の走査対象に
  含まれていなかった。
- 対応：`js/csvManager.js`に`buildCorrectionQuestionEntries(state, filter)`を新設し、
  `collectQuestions()`の戻り値に合流させた。訂正問題は
  `question`＝「元のお題（誤答例: ○○ の訂正問題）」、`answer`＝訂正内容、
  `note`＝解説、という形でCSV/JSON出力の列にマッピングされる。`id`は
  `correction_` + 復習スケジュールエントリのidとして一意性を確保した。
  ジャンル／道／ステージによる絞り込みは、`GameState.findStageContext()`で
  対象ステージの道を解決して判定している。
- **判断が必要な点（要レビュー）**：この対応は「出力（エクスポート）」のみに
  対応したものであり、出力したCSV/JSONを再度「取り込み（インポート）」しても、
  訂正問題として`reviewSchedules`側へ戻すことはしない（`id`が`correction_`始まりの
  ため、他の通常インポート処理とは重複しない設計だが、そのまま取り込むと
  「通常の問題」として道・ステージ配下に新規追加されてしまう点に注意。今回は
  要望が「出力されるように」だったため、インポート往復までは実装していない）。

### ⑥gacha_mobile.mp4／open_mobile.mp4が小さく表示される問題
- 前回セッションで`#cutscene-video`を`width:100%;height:100%;object-fit:contain;`
  に変更済みだったが、この指摘が再度あったため、`object-fit`を`contain`から
  `cover`に変更した。`contain`は動画とビューポートのアスペクト比の差により
  上下または左右に黒い余白ができ「小さく表示されている」ように見えることが
  あるため、`cover`にして画面いっぱいに隙間なく表示されるようにした
  （はみ出す部分は切り取られる）。

### ⑦終了ボタンで終了演出（close.mp4／close_mobile.mp4）を再生
- `js/app.js`に`exitWithCutscene()`（新規公開関数）を追加。
  `playCutscene("close", ...)`で横画面はclose.mp4・縦画面はclose_mobile.mp4を
  再生し、終了後に`window.close()`を試みる（多くのブラウザではスクリプトで
  開いていないタブは閉じられず失敗する）。閉じられなかった場合は
  `showBlackout()`で動画・フォールバック要素を隠したまま`#video-overlay`
  （全画面黒背景）だけを表示し続け、画面を暗転させた状態で終了とする。
- `js/router.js`のホーム画面「終了」ボタンから、DB保存確定→バックアップJSON
  書き出し→`App.exitWithCutscene()`の順で呼ぶように変更した。

### ⑧チュートリアルモードの導入
- 新規ファイル`js/tutorialSystem.js`を作成。6枚のスライド（ようこそ／未知の霧／
  知の探究／建築・キャラクター／コンパスとガチャ／ホーム画面）で構成する
  簡潔なモーダル形式のチュートリアルとした（仕様71章「長い説明文は禁止」との
  整合を優先し、実際の画面操作を模倣した長尺デモではなく要点説明のみに
  留めている）。「← 戻る」「次へ →」「スキップ」「とじる」で操作する。
- ログイン前（待ち受け画面）からも呼び出せるよう、GameState/Routerに依存しない
  独立オーバーレイ（`document.body`に直接追加）として実装した。
- 起動方法は⑩の要望どおり、待ち受け画面の「チュートリアル」ボタンを押した
  場合のみ（自動起動はしない）。

### ⑨待ち受け画面のボタン再配置
- 主ボタンを「ログイン」から「この端末の保存データで続ける」に変更（処理内容は
  従来の`handleContinueOnThisDeviceClick`のまま）。
- バックアップファイル（.json）からのログインは、画面右上に新設した
  「バックアップ」ボタンへ移動。その下に「現在のデバイス内のバックアップ
  ファイル（.json）を選択します」の説明文を配置した。

### ⑩待ち受け画面の中央配置・チュートリアルボタンの追加
- `css/style.css`の`.screen-title`を`position:fixed; inset:0;`の全画面レイヤーに
  変更し、`justify-content:center; align-items:center;`と合わせて、画面の向き
  （縦画面／横画面）や祖先要素（`#screen-container`等）のサイズ計算に左右されずに、
  タイトル・ログインボタン・グラデーションの中心が常にビューポートの中央に
  くるようにした。グラデーションも`circle at 50% 30%`から`circle at 50% 50%`へ
  変更し、視覚的な中心と一致させた。
- 「この端末の保存データで続ける」ボタンの下に「チュートリアル」ボタンを追加
  （⑧参照）。

### 動作確認方法（次のAI/開発者が実施すること。本セッションはブラウザ実行環境が
無く目視確認ができていないため、必ず実施すること）
1. ローカルサーバーで配信して開く。
2. 問題画面（Lv1）で「→」の付いた文字が紺色（#000080）になっていることを確認する。
3. わざと誤答してFail画面を出し、「カード」ボタンと下のボタン群の間に余白が
   あること、フラッシュカード画面で「正答」面に選択肢一覧が表示されること
   （Lv2四択・Lv3並び替えの場合）、カードを左右にスワイプしても切り替わること、
   「問題」の文字が紺色になっていることを確認する。
4. ホーム画面を縦画面・横画面それぞれで開き、「今日のおすすめ行動」と
   「世界の様子」が常に縦に並んでいることを確認する。
5. 1週間後の復習で訂正問題を作成した後、設定→データ管理→全問題／エリア別／
   道別／ステージ別のいずれかを出力し、作成した訂正問題が含まれていることを
   確認する。
6. 縦画面のスマートフォン（実機推奨）でopen/gachaの動画演出を確認し、
   画面いっぱいに表示されることを確認する。
7. ホーム画面の「終了」ボタンを押し、close/close_mobileの動画が再生された後、
   タブが閉じるか、閉じられない場合は画面が黒いまま操作不能になることを確認する。
8. 待ち受け画面で「チュートリアル」ボタンを押し、6枚のスライドが「次へ」
   「戻る」「スキップ」で操作できることを確認する（チュートリアルボタンを
   押さない限り自動では起動しないことも確認する）。
9. 待ち受け画面で、主ボタンが「この端末の保存データで続ける」になっており、
   右上の「バックアップ」ボタンからバックアップファイル（.json）を選んで
   ログインできることを確認する。
10. 待ち受け画面を縦画面・横画面それぞれで開き、タイトル・ボタン・グラデーションの
    中心が画面の中央に来ていることを確認する。

### 次回セッションへの申し送り
- ⑤の訂正問題エクスポートは出力専用（インポートで`reviewSchedules`側へ戻す
  往復処理は未実装）。往復対応が必要か確認したい。
- ⑧のチュートリアルは要点説明のスライド形式に留めた。仕様71章の趣旨
  （実際に操作しながら理解させる）をより厳密に実現したい場合、実際の画面
  遷移を伴うガイド付きツアー形式への拡張を検討されたい。
- ⑥⑦の動画関連はブラウザでの実機確認が引き続き必要（本セッションもネットワーク
  制限により未実施）。

## 18. 2026-09-19セッションでの追加実装（6項目の機能追加要望）

### 実装内容

1. **①操作体験型チュートリアル**: `js/tutorialSystem.js` を全面書き換え。従来の
   「絵文字＋説明文のみのスライド形式」を廃止し、実際の画面・実際のボタンを使って
   操作しながら理解できる形式に変更した。ホーム→探索→未知の霧→（地図で）ジャンル選択
   →道選択→（地図で）ステージ選択→ステージポップアップでLv1選択→実際に1問回答、という
   実際のゲーム操作を8ステップでガイドする。各ステップは対象要素を光る枠でハイライトし
   （`pointer-events:none` のため実際のクリックは邪魔しない）、`MutationObserver` と
   ポーリング（500ms）で画面遷移・ポップアップ出現・実解答を検知して自動的に次へ進む。
   いつでも「スキップ」で終了可能。
2. **②設定画面の再受講ボタン**: `renderSettingsScreen`（router.js）の最下部に
   「チュートリアル」ボタンを追加。押すと`TutorialSystem.start()`が再実行される。
3. **③正答面の実選択肢表示**: Lv1（正誤）はcorrectAnswerが真偽値のため、これまで
   結果画面（Fail画面の解答一覧・カード画面の「正答」面）で「正答」「誤答」という
   抽象的な文言がそのまま表示されていた。`item.answerText`（実際に出題された選択肢）が
   ある場合はそちらを優先して表示するよう `resultSystem.js`（renderFailScreen /
   buildAnswerFaceContent）と `reviewSystem.js`（☆フォルダ復習の completeStarReview
   呼び出し箇所）を修正。Lv2/Lv3はcorrectAnswerが既に実際の文字列のため表示は変わらない。
4. **④選択肢の縦並び**: `quizSystem.js` に `adjustChoiceLayoutIfOverflowing()` を追加。
   Lv1（正誤ボタン）・Lv2（四択）のボタンをDOM挿入後に一時的に `white-space:nowrap` に
   して `scrollWidth > clientWidth` を判定し、はみ出す（＝折り返しが発生する）場合のみ
   `.is-vertical-choices` クラスを付与して縦一列に並べ替える（quiz.css）。
5. **⑤キャラクターセリフ**: 新規 `js/characterVoiceSystem.js` を追加し、
   `data/serihu.json` を起動時に読み込む（AI生成なし、事前登録セリフのみ・仕様32章準拠）。
   問題画面（`quizSystem.js`）下部に常時キャラクターパネルを表示し、1問正解するごとに
   そのジャンルに割り当てられたセリフをランダムに1つ表示する。ジャンル名の表記ゆれ
   （画像ファイル名・英語表記など）は`characterVoiceSystem.js`内のALIAS対応表で吸収し、
   該当が無い場合は`serihu.json`内の`"default"`セリフにフォールバックする。
6. **⑥整理（クリーンアップ）機能**: `GameState.cleanupEmptyContent()`（gameState.js）を
   追加。ステージ数が1つ以下になった道（問題セット）を削除し、その結果道が0個になった
   ジャンルも削除する。削除された道・ステージに紐づく`stageProgress`／
   `reviewSchedules`／`reviewHistory`／`starQuestions`／`questionMistakes`／`state.map`
   の各エントリもあわせて削除し、矛盾したデータが残らないようにした。建築物・
   キャラクター・称号・コンパスは削除しない（損失回避の考え方を踏襲）。設定画面の
   データ管理に「整理」タブを新設し、実行ボタンを配置（confirm()による確認あり）。

### 判断が必要な点（要レビュー）

- **⑤のキャラクター数について**: 依頼文では「現在ある11のキャラクター」とあったが、
  アップロードされたZIP内には`assets/img/`に8つのジャンル系画像
  （English / Ethical / Information / Phirosophy / ReadingBook / WorldHistory /
  math / program）しか見当たらず、実際の11キャラクター分のデータ（保存済みIndexedDB）
  はこのZIPには含まれていないため、この8ジャンル分のみ`serihu.json`に作成した。
  残り3キャラクター（またはジャンル名の答え合わせ）を教えていただければ、
  次回`serihu.json`に追記します。ジャンル名がここに登録した8つと表記が異なる場合も、
  `characterVoiceSystem.js`のALIASES、または`serihu.json`本体のキー名を実際のジャンル名
  に合わせて調整してください（現状は"default"セリフにフォールバックするため、動作自体は
  壊れない）。
- **①のSVG地図ステップについて**: `fogStageSelect`（ジャンル地図）と`areaMap`
  （ステージ地図）はパン・ズーム可能なSVGのため、内部の図形（`.map-area-group` /
  `.map-stage-group`）を`getBoundingClientRect()`でハイライトしているが、パン・ズーム後の
  座標変化への追従は簡易的なもの（500msポーリング＋MutationObserverでの再計算）。
  ズーム・パン中に多少ハイライト位置がずれる可能性があるが、進行判定自体は画面遷移の
  検知（ハイライトとは独立）で行っているため、操作の完了自体には影響しない。
- ジャンルが1つも登録されていない状態でチュートリアルが開始された場合、地図タップの
  体験ができないため、その旨の案内文＋「次へ」ボタンで最終ステップへスキップする分岐を
  追加した。

### 動作確認

- 全JSファイルを`node --check`で検証、全CSSファイルの波括弧バランスを確認、
  `serihu.json`のJSON妥当性を確認。いずれも問題なし。
- ブラウザ実機での動作確認（チュートリアルのハイライト位置・実際のクリック検知・
  キャラクターパネルの表示・整理タブの削除挙動など）は引き続きサンドボックスでは
  実施できないため、Saito側での確認をお願いします。
